...
double Dot(VD &vec1, VD &vec2 , mpiInfo &myMPI)
{
  double local_dot = 0.;
    ...
    // 1. Create a vector of ones
    VD onesVec;
    onesVec.resize(vec1.size(), 1.0); //since vec1 and vec2 are the same size doesn't matter which one we do
    ...
    //   }
    // }
    // 2. Use PEsum on this vector
    myMPI.PEsum(onesVec); 

    // Compute the dot product on this PE.
    rLOOP 
    {
        // 3. Adjust each entry by dividing by the count (i.e., the value in onesVec)
  ...
  while ( global_converged == 0  && ++iter <= max_iter)
    {
      ...
      rowLOOP {
	Solution[row] = Solution[row] + alpha *  p[row];
