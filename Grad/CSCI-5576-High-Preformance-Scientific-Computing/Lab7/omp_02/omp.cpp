...
#pragma omp parallel shared(numTHconverged)
  ...
  //int i = pt / nPtsy;
	  ...
	  if ( j > 0 && j < nPtsy-1 )
            {
              // The point of these "TO-DOs" is to make sure you understand how to compute the row number for this thread.
	      
	      
              Acoef[ pt_TH ][0] = -4./dh/dh;  Jcoef[pt_TH ][0] = pt;             
              Acoef[ pt_TH ][1] = Acoef[ pt_TH ][2] = Acoef[ pt_TH ][3] = Acoef[ pt_TH ][4] =  1./dh/dh;
	      Jcoef[ pt_TH ][1] = pt - 1;        
              Jcoef[ pt_TH ][2] = pt + 1;        
	...
	if (i==0) { Acoef[ pt_TH ][0] = 1. ; Jcoef[ pt_TH ][0] = pt; b[ pt_TH ] =  1.; }
        ...
        else if ( j == nPtsy-1 ) { Acoef[ pt_TH ][0] = 1. ; Jcoef[ pt_TH ][0] = pt; b[ pt_TH ] =  1.; }
      }
    ...
    while ( numTHconverged < numTH && ++iter <= max_iter )   // The TO-DO is the global convergence criteria
      {
        ...
        THconverged[myTH] = thisThreadConverged;      // For this TO-DO, enter the variable that stores the convergence status for all threads.
