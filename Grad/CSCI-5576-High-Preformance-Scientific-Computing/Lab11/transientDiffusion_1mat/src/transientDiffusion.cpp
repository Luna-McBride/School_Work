   ...
   for (int count =  0 ; count < argc; ++count)
     {
       ...
       if ( !strcmp(argv[count],"-restart" ) ) restart = atoi(argv[count+1]);   
     }
   ...
   if ( restart )
     {
       ...
       MESH.plot( "load_phi"  , MESH.phi ,     count , myMPI      );
       // Account for the fact that values are saved at the
       // end of the loop (i.e. post update for that loop)
       tStart += dt;
     }
