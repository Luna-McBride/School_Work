...
public:
  ...
  void ParticlesOnMesh(particles &PTCL, mpiInfo &myMPI)
  {
    ...
    if ( myMPI.myPE == 0 ) { for ( int k = 1 ; k <= nField ; ++k ) Qval[k] = 1.; }
    if ( myMPI.myPE == 1 ) { for ( int k = 1 ; k <= nField ; ++k ) Qval[k] = 2.; }
    if ( myMPI.myPE == 2 ) { for ( int k = 1 ; k <= nField ; ++k ) Qval[k] = 3.; }
    if ( myMPI.myPE == 3 ) { for ( int k = 1 ; k <= nField ; ++k ) Qval[k] = 4.; }
