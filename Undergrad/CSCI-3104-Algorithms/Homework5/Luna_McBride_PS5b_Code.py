...
def union(fv1,fv2):
    ...
    cv2=components[fv2]
    if len(cv1) <= len(cv2):
        components[fv2]=components[fv2]+components[fv1]
        for i in range(0,len(components[fv1])):
            leader_dict[components[fv1][i]]=fv2
    else:
        components[fv1]=components[fv1]+components[fv2]
        for i in range(0,len(components[fv2])):
            leader_dict[components[fv2][i]]=fv1
...
if __name__ == '__main__':
    ...
    for i in range(0,len(sorted_edges)):
        if len(kruskal_selected_edges)>= (len(vertices)-k-1):
            #for i in range(i,len(sorted_edges)):
            #    v1=sorted_edges[i][0]
            #    v2=sorted_edges[i][1]
            #    if find(v1)!=find(v2):
        ...
        if find(v1)!=find(v2):
            union(find(v1),find(v2))
            kruskal_selected_edges=kruskal_selected_edges+[sorted_edges[i]]
    ...
    #Code used to get the connections changed:
    #for i in range(i,len(sorted_edges)):
    #            v1=sorted_edges[i][0]
    #            v2=sorted_edges[i][1]
    #            if find(v1)!=find(v2):
